...
function addStadium() {
  ...
  var mesh = new THREE.SceneUtils.createMultiMaterialObject(new THREE.PlaneGeometry(8240, 12000), materials);
...
function addCar(name, actor) {
  ...
  // renderRect(74, 144),
  new THREE.BoxGeometry(90, 175, 1), new THREE.MeshLambertMaterial({
    map: texture,
    transparent: true,
    opacity: 1
  }));
  ...
  */
  ...
  if (actorData[actor.id] !== undefined) {
    var team = teamData[actorData[actor.id].team];
  ...
  carsLoading = carsLoading.filter(function (e) {
    return e !== name;
  });
}
...
function addBall(name, actor) {
  ...
  var mesh = new THREE.Mesh(new THREE.CircleGeometry(91, 32), new THREE.MeshLambertMaterial({
